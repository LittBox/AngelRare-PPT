<script setup>
const sectionIndex = '01'
const sectionTitle = '个人成长'
const subtitle = 'Personal Growth'
</script>
...
<template>
  <div class="slide-chapter">
    ...
    <div class="content">
      <div class="chapter-index-bg">{{ sectionIndex }}</div>
      <div class="chapter-content">
        <div class="chapter-num">{{ sectionIndex }}</div>
        <h1 class="chapter-title">{{ sectionTitle }}</h1>
...
.bg-base {
  ...
  background: radial-gradient(ellipse 100% 60% at 50% 0%, #040819 0%, #020817 100%);
}
.bg-orb-1 {
...
.chapter-index-bg {
  position: absolute;
  font-size: 600px; font-weight: 900; font-family: var(--font-mono);
  color: rgba(57,141,255,0.04);
...
.chapter-num {
  ...
  background: linear-gradient(135deg, #3C8DFF 0%, #19C6FF 100%);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
  font-family: var(--font-mono); letter-spacing: 0px; margin-bottom: 24px;
}
.chapter-title {
  font-size: var(--text-5xl); font-weight: 800; letter-spacing: 24px; margin: 0 0 24px 0;
  background: linear-gradient(135deg, #ffffff 0%, #c8d8ff 100%);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
}
.chapter-subtitle {
  font-size: var(--text-xl); color: rgba(255,255,255,0.40);
